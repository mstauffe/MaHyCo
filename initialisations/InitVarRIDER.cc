...
void Initialisations::initVarRiderMono(RealArray1D<dim> Xb) noexcept {
  Kokkos::parallel_for(
      "initVarShockBubble", nbCells, KOKKOS_LAMBDA(const int& cCells) {
        ...
        if (test->Nom == test->MonoRiderTx) m_cell_velocity_n0(cCells) = {{1., 0.}};
        if (test->Nom == test->MonoRiderTy) m_cell_velocity_n0(cCells) = {{0., 1.}};
        if (test->Nom == test->MonoRiderT45)
          m_cell_velocity_n0(cCells) = {{1., 1.}};
        if (test->Nom == test->MonoRiderRotation) {
        ...
        for (size_t pNodesOfCellC = 0; pNodesOfCellC < nbNodesOfCellC;
          ...
          double rnode = std::sqrt((m_node_coord_n0(pNodes)[0] - Xb[0]) *
                                       (m_node_coord_n0(pNodes)[0] - Xb[0]) +
                                   (m_node_coord_n0(pNodes)[1] - Xb[1]) *
                                       (m_node_coord_n0(pNodes)[1] - Xb[1]));
  ...
  Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNodes) {
    RealArray1D<dim> cc = {{0.5, 0.5}};
    if (test->Nom == test->MonoRiderTx) m_node_velocity_n0(pNodes) = {{1., 0.}};
    if (test->Nom == test->MonoRiderTy) m_node_velocity_n0(pNodes) = {{0., 1.}};
    if (test->Nom == test->MonoRiderT45) m_node_velocity_n0(pNodes) = {{1., 1.}};
    if (test->Nom == test->MonoRiderRotation) {
...
void Initialisations::initVarRider(RealArray1D<dim> Xb) noexcept {
  Kokkos::parallel_for(
      "initVarShockBubble", nbCells, KOKKOS_LAMBDA(const int& cCells) {
        ...
        double rb(0.15);
        RealArray1D<dim> v_init;
        if (test->Nom == test->RiderTx) m_cell_velocity_n0(cCells) = {{1., 0.}};
        if (test->Nom == test->RiderTy) m_cell_velocity_n0(cCells) = {{0., 1.}};
  ...
  Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNodes) {
    RealArray1D<dim> cc = {{0.5, 0.5}};
    if (test->Nom == test->RiderTx) m_node_velocity_n0(pNodes) = {{1., 0.}};
    if (test->Nom == test->RiderTy) m_node_velocity_n0(pNodes) = {{0., 1.}};
    if (test->Nom == test->RiderT45) m_node_velocity_n0(pNodes) = {{1., 1.}};
    if (test->Nom == test->RiderRotation) {
      RealArray1D<dim> dd = m_node_coord_n0(pNodes) - cc;
      double theta = std::atan2(dd[1], dd[0]);
      double r = std::sqrt(dd[0] * dd[0] + dd[1] * dd[1]);
