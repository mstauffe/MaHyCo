  ...
  Kokkos::parallel_for(
      "initPart", nbPart, KOKKOS_LAMBDA(const int& ipart) {
        ...
        if (m_particle_coord_nplus1(ipart)[1] < 0.) {
          m_particle_coord_nplus1(ipart)[1] =
              -m_particle_coord_nplus1(ipart)[1];
          m_particle_velocity_n(ipart)[1] = -m_particle_velocity_n(ipart)[1];
        }
        int icell = MathFunctions::max(
            floor(m_particle_coord_nplus1(ipart)[0] / cstmesh->X_EDGE_LENGTH),
            0);
        int jcell = MathFunctions::max(
            floor(m_particle_coord_nplus1(ipart)[1] / cstmesh->Y_EDGE_LENGTH),
            0);
        m_particle_cell(ipart) = jcell * cstmesh->X_EDGE_ELEMS + icell;
        ...
        if (m_particlecell_fracvol_env(
          ...
          if (m_particle_env(ipart) == 0)
            gradf =
                m_particlecell_fracvol_gradient_env(m_particle_cell(ipart), 0);
          if (m_particle_env(ipart) == 1)
            gradf =
                m_particlecell_fracvol_gradient_env(m_particle_cell(ipart), 1);
          if (m_particle_env(ipart) == 2)
            gradf =
                m_particlecell_fracvol_gradient_env(m_particle_cell(ipart), 2);
          ...
          double module_vit = sqrt(m_particle_velocity_n(ipart)[0] *
                                       m_particle_velocity_n(ipart)[0] +
                                   m_particle_velocity_n(ipart)[1] *
                                       m_particle_velocity_n(ipart)[1]);
          double module_gradf = sqrt(gradf[0] * gradf[0] + gradf[1] * gradf[1]);
          m_particle_velocity_n(ipart) =
              gradf * 1.1 * module_vit / module_gradf;
          m_particle_coord_nplus1(ipart) =
              m_particle_coord_n(ipart) +
              m_particle_velocity_n(ipart) * gt->deltat_n;
          if (m_particle_coord_nplus1(ipart)[1] < 0.) {
  ...
  Kokkos::parallel_for(
      "initPart", nbPart, KOKKOS_LAMBDA(const int& ipart) {
        ...
        m_particlecell_velocity_nplus1(icells)[0] +=
            m_particle_mass(ipart) * AccelerationP[0] /
            m_particlecell_mass(icells);
        m_particlecell_velocity_nplus1(icells)[1] +=
            m_particle_mass(ipart) * AccelerationP[1] /
            m_particlecell_mass(icells);
      });
}
}  // namespace particleslib
