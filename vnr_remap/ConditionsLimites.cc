...
void Vnr::updateVelocityBoundaryConditions() noexcept {
  ...
  Kokkos::parallel_for(
      nbBottomNodes, KOKKOS_LAMBDA(const size_t& pBottomNodes) {
                  ...
                  sumR1(reduction1,
                        (m_pressure_n(cCells) + m_pseudo_viscosity_n(cCells)) *
                            (m_cqs(cCells, pNodesOfCellC) +
                             symmetricVector(m_cqs(cCells, pNodesOfCellC),
                                             {1.0, 0.0})));
            }
          }
          m_node_velocity_nplus1(pNodes) =
              m_node_velocity_n(pNodes) +
              dt / (m_node_mass(pNodes)) * reduction1;
  ...
  Kokkos::parallel_for(
      nbTopNodes, KOKKOS_LAMBDA(const size_t& pTopNodes) {
        ...
        if (cdl->topBC == cdl->symmetry) {
          ...
          m_node_velocity_nplus1(pNodes) =
              m_node_velocity_n(pNodes) +
              dt / (m_node_mass(pNodes)) * reduction2;
  ...
  Kokkos::parallel_for(
      nbLeftNodes, KOKKOS_LAMBDA(const size_t& pLeftNodes) {
        ...
        if (cdl->leftBC == cdl->symmetry) {
          ...
          m_node_velocity_nplus1(pNodes) =
              m_node_velocity_n(pNodes) +
              dt / (m_node_mass(pNodes)) * reduction3;
  ...
  Kokkos::parallel_for(
      nbRightNodes, KOKKOS_LAMBDA(const size_t& pRightNodes) {
        ...
        if (cdl->rightBC == cdl->symmetry) {
          ...
          m_node_velocity_nplus1(pNodes) =
              m_node_velocity_n(pNodes) +
              dt / (m_node_mass(pNodes)) * reduction4;
...
void Vnr::updatePeriodicBoundaryConditions() noexcept {
  Kokkos::parallel_for(
      nbCells, KOKKOS_LAMBDA(const size_t& cCell) {
        if (cdl->rightCellBC == cdl->periodic) {
          ...
          if (LeftCell != -1) {
            ...
            m_internal_energy_nplus1(cCell) =
