...
RealArray1D<2> computeLpcNpc(RealArray1D<2> xp, RealArray1D<2> xpPlus,
  ...
  return computeLpcPlus(xp, xpPlus) * computeNpcPlus(xp, xpPlus) +
         computeLpcPlus(xpMinus, xp) * computeNpcPlus(xpMinus, xp);
}
...
template <size_t x>
KOKKOS_INLINE_FUNCTION RealArray1D<x> sumR1(RealArray1D<x> a,
                                            RealArray1D<x> b) {
  return a + b;
}

KOKKOS_INLINE_FUNCTION
double sumR0(double a, double b) { return a + b; }

KOKKOS_INLINE_FUNCTION
double minR0(double a, double b) { return std::min(a, b); }
