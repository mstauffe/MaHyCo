  ...
  if(mesdonnees)
    {
      ...
      std::cout << " Cas test " << mot << " ( " << o->testCase <<  " ) " << std::endl;
      mesdonnees.ignore();
      ...
      getline(mesdonnees, ligne); // ligne de commentaire Temps entre deux sorties
      ...
      // std::cout << " Equation d'etat " << mot << " ( " << o->eos << " ) " << std::endl;
      // mesdonnees.ignore();

      // getline(mesdonnees, ligne); // Equilibrage des pressions 
      // mesdonnees >> mot;
      // o->AvecEquilibrage = equilibrage[mot];
      // std::cout << " Equilibrage des pressions " << mot << " ( " << o->AvecEquilibrage << " ) " << std::endl;
      // mesdonnees.ignore();
      ...
      // std::cout << " schema Lagrange " << mot << " ( " << o->schemaLagrange << " ) " << std::endl;
      // mesdonnees.ignore();

      getline(mesdonnees, ligne); // ordre en espace du schema Lagrange
      mesdonnees >> o->spaceOrder;
      std::cout << " Ordre en espace du schéma Lagrange  " << o->spaceOrder << std::endl;
      mesdonnees.ignore();

      // getline(mesdonnees, ligne); // Avec projection
      // mesdonnees >> mot;
      // o->AvecProjection = ouiOUnon[mot];
      // std::cout << " Avec Projection " << mot << " ( " << o->AvecProjection << " ) " << std::endl;
      // mesdonnees.ignore();

      getline(mesdonnees, ligne); // Projection conservative
      mesdonnees >> mot;
      o->projectionConservative = ouiOUnon[mot];
      std::cout << " Projection conservative " << mot << " ( " << o->projectionConservative << " ) " << std::endl;
      mesdonnees.ignore();

      getline(mesdonnees, ligne); // Limite de fraction volumique des mailles mixtes
      mesdonnees >> o->threshold;
      std::cout << " Limite de fraction volumique des mailles mixtes  " << o->threshold << std::endl;
      mesdonnees.ignore();
      
      getline(mesdonnees, ligne); // ordre de la projection
      mesdonnees >> o->projectionOrder;
      std::cout << " Ordre en de la phase de projection  " << o->projectionOrder << std::endl;
      mesdonnees.ignore();
      
      getline(mesdonnees, ligne); // Limiteur
      mesdonnees >> mot;
      o->projectionLimiterId = limiteur[mot];
      std::cout << " Limiteur " << mot << " ( " << o->projectionLimiterId << " ) " << std::endl;
      mesdonnees.ignore();
     
      getline(mesdonnees, ligne); // Projection Avec Plateau Pente
      mesdonnees >> mot;
      o->projectionAvecPlateauPente = ouiOUnon[mot];
      std::cout << " Projection Avec Plateau Pente " << mot << " ( " << o->projectionAvecPlateauPente << " ) " << std::endl;
      mesdonnees.ignore();
      
      getline(mesdonnees, ligne); // Projection Avec Plateau Pente Mixte
      mesdonnees >> mot;
      o->projectionLimiteurMixte= ouiOUnon[mot];
      std::cout << " Projection Avec Plateau Pente Mixte " << mot << " ( " << o->projectionLimiteurMixte << " ) " << std::endl;
      mesdonnees.ignore();

      getline(mesdonnees, ligne); // Projection Limiteur pour Mailles Pures
      mesdonnees >> mot;
      o->projectionLimiterIdPure = limiteur[mot]; 
      std::cout << " Limiteur pour Mailles Pures " << mot << " ( " << o->projectionLimiterIdPure << " ) " << std::endl;  
      mesdonnees.ignore();

      // getline(mesdonnees, ligne); // Presence de Particules
      // mesdonnees >> mot;
      // o->AvecParticules = ouiOUnon[mot];
      // std::cout << " Presence de Particules " << mot << " ( " << o->AvecParticules << " ) " << std::endl;
      // mesdonnees.ignore();
