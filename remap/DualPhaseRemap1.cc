...
void Remap::computeDualUremap1() noexcept {
  if (varlp->x_then_y_n) {
    if (options->projectionOrder > 1) {
      ...
      Kokkos::parallel_for(
          nbNodes, KOKKOS_LAMBDA(const size_t& pNode) {
      ...
      Kokkos::parallel_for(
          nbNodes,
          KOKKOS_LAMBDA(const size_t& pNode) { gradDualPhi1(pNode) = Uzero; });
    }
    Kokkos::parallel_for(
        nbNodes, KOKKOS_LAMBDA(const size_t& pNode) {
  ...
  Kokkos::parallel_for(
      nbNodes, KOKKOS_LAMBDA(const size_t& pNode) {
        ...
        varlp->DualPhi(pNode)[1] =
            UDualremap1(pNode)[1] / UDualremap1(pNode)[2];
        // masse nodale
        varlp->DualPhi(pNode)[2] = UDualremap1(pNode)[2];
        // energie cinétique
        varlp->DualPhi(pNode)[3] =
            UDualremap1(pNode)[3] / UDualremap1(pNode)[2];
