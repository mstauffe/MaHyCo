  ...
  if (varlp->x_then_y_n) {
	...
	for (int imat = 0; imat < nbmat; imat++) {
	  ...
	  if (VerticalFaceOfNode(pNodes)[0] != -1) {
	    ...
	    // std::cout << " pNodes " <<  pNodes << " sortie 1 remaillage masse N " << std::endl;
	    // std::cout << " Right FluxFace1(cfCell1, fOfcfCell1)[nbmat+imat] " << FluxFace1(cfCell1, fOfcfCell1)[nbmat+imat]
	    // 	      << "  FluxFace1(cfCell1, frOfcfCell1)[nbmat+imat] " <<  FluxFace1(cfCell1, frOfcfCell1)[nbmat+imat]
	    // 	      << std::endl;
	    //  std::cout << " Left FluxFace1(cbCell1, fOfcbCell1)[nbmat+imat] " << FluxFace1(cbCell1, fOfcbCell1)[nbmat+imat]
	    // 	       << " FluxFace1(cbCell1, flOfcbCell1)[nbmat+imat] " << FluxFace1(cbCell1, flOfcbCell1)[nbmat+imat]
	    // 	       << std::endl;
	...
	// std::cout << " pNodes " <<  pNodes << " sortie 1 remaillage masse N " << varlp->UDualLagrange(pNodes)[2]
	//  	  << "  " << UDualremap1(pNodes)[2] << " gauche " << LeftFluxMasse(pNodes) << " droite "
	// 	  << RightFluxMasse(pNodes) << " et nbfaces " << nbfaces << std::endl;
	...
	UDualremap1(pNodes)[1] = varlp->UDualLagrange(pNodes)[1]
	   ...
	   // std::cout << gt->deltat_n << " pNodes " <<  pNodes << " sortie 1 remaillage vitesse "
	   // 	    << varlp->UDualLagrange(pNodes)[0]/varlp->UDualLagrange(pNodes)[2]
	   // 	    << " quant  "
	   // 	    << varlp->UDualLagrange(pNodes)[0] << "  "
	   // 	    << UDualremap1(pNodes)[0] << " lfm " << LeftFluxMasse(pNodes)
	...
	if (HorizontalFaceOfNode(pNodes)[0] !=-1) {
	  ...
	  int fFace1(utils::indexOf(mesh->getFaces(), fId1));  
	...
	if (HorizontalFaceOfNode(pNodes)[1] !=-1) {
	  ...
	  // on recupere la cellule au dessus "cb"
	  int cbBackCellF2(mesh->getBackCell(fId2));
	  int cbId2(cbBackCellF2);
	  cbCell2 = cbId2;
	  // on recupere la face au dessus de la cellule au dessus
	...
	// std::cout << " pNodes " <<  pNodes << " avant proj1V " << varlp->UDualLagrange(pNodes)[2]
	//	  << " apres proj1V " << UDualremap1(pNodes)[2] << std::endl;
	if (options->projectionOrder == 1) {
	  // recherche de la vitesse du decentrement upwind
