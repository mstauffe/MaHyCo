...
void Remap::computeDualUremap1() noexcept {
  if (varlp->x_then_y_n) {
      ...
      Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNode)
	{
	  ...
	  gradDualPhi1(pNode) = computeDualHorizontalGradPhi(grad_right, grad_left, pNode);
	});
    } else {
       Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNode)
	{
	...
	if (options->projectionOrder >= 1) {	  
	  ...
	  getLeftUpwindVelocity(LeftNode, pNode, gradDualPhi1(LeftNode), gradDualPhi1(pNode));
	  getRightUpwindVelocity(RightNode, pNode, gradDualPhi1(RightNode), gradDualPhi1(pNode));
	  
	  UDualremap1(pNode)[0] = varlp->UDualLagrange(pNode)[0]
	    + LeftFluxMasse(pNode) * LeftupwindVelocity(pNode)[0]
	    - RightFluxMasse(pNode) * RightupwindVelocity(pNode)[0];
	
	  UDualremap1(pNode)[1] = varlp->UDualLagrange(pNode)[1]
      ...
      Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNode)
	{
	  ...
	  gradDualPhi1(pNode) = computeDualVerticalGradPhi(grad_up, grad_down, pNode);
	});
    } else {
       Kokkos::parallel_for(nbNodes, KOKKOS_LAMBDA(const size_t& pNode)
	{
	...
	if (options->projectionOrder >= 1) {
	  ...
	  getBottomUpwindVelocity(BottomNode, pNode, gradDualPhi1(BottomNode), gradDualPhi1(pNode));
	  getTopUpwindVelocity(TopNode, pNode, gradDualPhi1(TopNode), gradDualPhi1(pNode));
