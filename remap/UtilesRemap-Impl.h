...
RealArray1D<d> Remap::computeDualVerticalGradPhi(RealArray1D<d> grad_top,
  ...
  grad_bottom[0] =
      (varlp->DualPhi(pNode)[0] - varlp->DualPhi(BottomNode)[0]) /
  ...
  grad_bottom[1] =
      (varlp->DualPhi(pNode)[1] - varlp->DualPhi(BottomNode)[1]) /
      (varlp->XLagrange(pNode)[1] - varlp->XLagrange(BottomNode)[1]);
  ...
  if (BottomBottomNode == -1) {
    hmoins = 0.;
    hplus =
        0.5 * (varlp->XLagrange(TopTopNode)[1] - varlp->XLagrange(pNode)[1]);
  } else if (TopTopNode == -1) {
    hplus = 0.;
    hmoins = 0.5 * (varlp->XLagrange(pNode)[1] -
                    varlp->XLagrange(BottomBottomNode)[1]);
  } else {
    hmoins = 0.5 * (varlp->XLagrange(pNode)[1] -
                    varlp->XLagrange(BottomBottomNode)[1]);
    hplus =
  ...
  RealArray1D<d> res;
  res = computeAndLimitGradPhi(limiter, grad_top, grad_bottom,
                               varlp->DualPhi(pNode), varlp->DualPhi(TopNode),
                               varlp->DualPhi(BottomNode), h0, hplus, hmoins);
...
RealArray1D<d> Remap::computeDualHorizontalGradPhi(RealArray1D<d> grad_right,
  ...
  if (LeftLeftNode == -1) {
    hmoins = 0.;
    hplus = 0.5 *
            (varlp->XLagrange(RightRightNode)[0] - varlp->XLagrange(pNode)[0]);
  } else if (RightRightNode == -1) {
    hplus = 0.;
    hmoins =
        0.5 * (varlp->XLagrange(pNode)[0] - varlp->XLagrange(LeftLeftNode)[0]);
  } else {
    hmoins =
        0.5 * (varlp->XLagrange(pNode)[0] - varlp->XLagrange(LeftLeftNode)[0]);
    hplus = 0.5 *
  ...
  RealArray1D<d> res;
  res = computeAndLimitGradPhi(limiter, grad_right, grad_left,
                               varlp->DualPhi(pNode), varlp->DualPhi(RightNode),
                               varlp->DualPhi(LeftNode), h0, hplus, hmoins);
...
RealArray1D<d> Remap::computeRemapFlux(
  ...
  if (projectionAvecPlateauPente == 0) {
    ...
    if (MathFunctions::fabs(dot(face_normal, exy)) < 1.0E-10)
      return (0.0 * phi_face);
    return (dot(outer_face_normal, exy) * face_normal_velocity * face_length *
            deltat_n * phi_face);
  } else {
