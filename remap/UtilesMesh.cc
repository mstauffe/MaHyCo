...
void Remap::FacesOfNode() {
  ...
  Kokkos::parallel_for(
      "computenodalfluxes", nbInnerVerticalFaces,
      KOKKOS_LAMBDA(const int& fInnerVerticalFaces) {
        ...
        if (VerticalFaceOfNode(n1Nodes)[0] == -1)
          VerticalFaceOfNode(n1Nodes)[0] = fId;
        else
          VerticalFaceOfNode(n1Nodes)[1] = fId;

        if (VerticalFaceOfNode(n2Nodes)[0] == -1)
          VerticalFaceOfNode(n2Nodes)[0] = fId;
        else
          VerticalFaceOfNode(n2Nodes)[1] = fId;
      });
  auto innerHorizontalFaces(mesh->getInnerHorizontalFaces());
  int nbInnerHorizontalFaces(mesh->getNbInnerHorizontalFaces());
  Kokkos::parallel_for(
      "computenodalfluxes", nbInnerHorizontalFaces,
      KOKKOS_LAMBDA(const int& fInnerHorizontalFaces) {
        ...
        if (HorizontalFaceOfNode(n1Nodes)[0] == -1)
          HorizontalFaceOfNode(n1Nodes)[0] = fId;
        else
          HorizontalFaceOfNode(n1Nodes)[1] = fId;

        if (HorizontalFaceOfNode(n2Nodes)[0] == -1)
          HorizontalFaceOfNode(n2Nodes)[0] = fId;
        else
          HorizontalFaceOfNode(n2Nodes)[1] = fId;
      });
  // Verification
  ...
  //       int n2Id(n2SecondNodeOfFaceF);
  //       int n2Nodes(n2Id);
  // 	std::cout << " face " << fFaces << " n1Nodes " <<  n1Nodes << " n2Nodes
  // " << n2Nodes << std::endl;
  //     });
  //   const auto innerNodes(mesh->getInnerNodes());
  //   const size_t nbInnerNodes(mesh->getNbInnerNodes());
  //   Kokkos::parallel_for(nbInnerNodes, KOKKOS_LAMBDA(const size_t&
  //   pInnerNodes)
  //     {
  // 	const Id pId(innerNodes[pInnerNodes]);
  // 	const size_t pNodes(pId);
  //     std::cout << " pNodes " <<  pNodes << " 1.H " <<
  //     HorizontalFaceOfNode(pNodes)[0]
  // 		<< " 2.H " << HorizontalFaceOfNode(pNodes)[1] << std::endl;
  //     std::cout << " pNodes " <<  pNodes << " 1.V " <<
  //     VerticalFaceOfNode(pNodes)[0]
